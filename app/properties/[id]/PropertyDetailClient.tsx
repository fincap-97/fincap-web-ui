...
// // ── CONFIG — apna number yahan daalo ──
// const CONTACT_PHONE = '+91 96966 61530'
// const COMPANY_NAME = 'Fincap Adviser'
...
//   // ── Related: closest price range, different developer, 1 per developer ──
//   const related = properties
//     .filter((p) =>
//       p.slug !== property.slug &&
//       p.developer !== property.developer
//     )
//     .sort((a, b) =>
//       Math.abs(a.priceValue - property.priceValue) -
//       Math.abs(b.priceValue - property.priceValue)
//     )
//     .reduce<typeof properties>((acc, p) => {
//       const key = p.developer ?? p.title
//       const already = acc.some(x => (x.developer ?? x.title) === key)
...
//       <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
//         <Link
//           href={`/properties?project=${encodeURIComponent(property.developer ?? property.title)}`}
//           className="inline-flex items-center gap-2 text-sm text-charcoal-muted hover:text-gold transition-colors mb-6 sm:mb-8"
//         >
//           <ArrowLeft className="w-4 h-4" />
//           Back to Properties
...
// // ── CONFIG — apna number yahan daalo ──
// const CONTACT_PHONE = '+919696661530'
// const COMPANY_NAME = 'Fincap Solutions'
...
//         <a
//           href={`/properties?project=${encodeURIComponent(property.developer ?? property.title)}`}
//           className="inline-flex items-center gap-2 font-semibold text-sm mb-6 sm:mb-8 px-4 py-2.5 rounded-xl transition-all"
//           style={{ background: 'transparent', color: '#E63946', border: '1.5px solid #E63946' }}
//           onMouseEnter={e => {
...
//                   </button>
//                 </div>
//                 {/* ── PRICE REMOVED from image overlay — only area shown ── */}
//                 <div className="absolute bottom-5 sm:bottom-6 left-5 sm:left-6 z-10">
//                   <p className="text-white/80 text-sm">{property.area}</p>
//                 </div>
//               </div>
//               <div className="flex gap-2 sm:gap-3 mt-3 sm:mt-4">
//                 {gradients.map((g, i) => (
...
//               {/* Enquiry Card — PRICE REMOVED from here too */}
//               <div className="bg-white rounded-xl sm:rounded-2xl p-5 sm:p-7 shadow-card-hover border border-stone-border/30">
//                 <div className="flex items-start justify-between mb-2">
//                   <div>
//                     {/* ── PRICE HIDDEN — only area + type shown ── */}
//                     <p className="text-charcoal-muted text-xs sm:text-sm">{property.area}</p>
//                   </div>
//                   <span className={`text-xs font-semibold px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full ${property.type === 'rent' ? 'bg-blue-100 text-blue-700'
//                     : property.type === 'project' ? 'bg-emerald-100 text-emerald-700'
//                       : 'bg-gold/10 text-gold'
...
//                         </span>
//                       </div>
//                       {/* ── PRICE REMOVED from similar cards — only area shown ── */}
//                       <div className="absolute bottom-4 left-4">
//                         <p className="text-white/80 text-xs mt-0.5">{p.area}</p>
//                       </div>
//                     </div>
//                     <div className="p-4 sm:p-5">
//                       <h3 className="font-serif font-semibold text-sm sm:text-base text-charcoal group-hover:text-gold transition-colors line-clamp-1">
...
// // ── CONFIG ──
// const CONTACT_PHONE = '+919696661530'
// const COMPANY_NAME = 'Fincap Advisors'
...
//   // Pad gallery to 4 items if fewer
//   const images = hasImages
//     ? [...(property.gallery!), ...(property.gallery!), ...(property.gallery!)].slice(0, 4)
//     : []
...
//             <div>
//               {/* Main Display */}
//               <div className="w-full h-64 sm:h-80 md:h-[460px] rounded-2xl sm:rounded-3xl overflow-hidden relative transition-all duration-500">
...
//                           />
//                           <div className="absolute inset-0" style={{ background: 'linear-gradient(180deg, rgba(0,0,0,0.1) 0%, transparent 40%, rgba(0,0,0,0.3) 100%)' }} />
//                         </>
//                       ) : (
//                         <div
...
//                         <p className="text-white/80 text-xs">{p.area}</p>
//                       </div>
//                     </div>
//                     <div className="p-4 sm:p-5">
//                       <h3 className="font-serif font-semibold text-sm sm:text-base text-charcoal group-hover:text-gold transition-colors line-clamp-1">
              ...
              {property.highlights.length > 0 && (
                <div className="grid grid-cols-2 gap-3 sm:gap-4 mt-5 sm:mt-6">
                  {property.highlights
                    ...
                    .sort((a, b) => {
                      if (a.label === 'RERA') return -1
                      if (b.label === 'RERA') return 1
                      return 0
                    })
                    .map((h) => (
                      <div key={h.label} className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 bg-ivory rounded-lg sm:rounded-xl">
